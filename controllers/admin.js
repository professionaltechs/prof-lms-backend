...
const getAllStudents = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error in get all students controller", data: error });
  }
};
...
const getAllFaculty = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error in get all faculty controller", data: error });
  }
};
...
const createCourse = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error in create course controller", data: error });
  }
};
...
const addStudentCourses = async (req, res) => {
  try {
    ...
    const updatedCourses = await studentModel.findByIdAndUpdate(
      studentId,
      { courses: allCourses },
      { new: true }
    );
    return res.json({
      ...
      data: { courses: updatedCourses.courses },
    });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({
        message: "Error in add student courses controller",
        data: error,
      });
  }
};

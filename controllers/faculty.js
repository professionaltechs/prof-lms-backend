...
const getCourseStudents = async (req, res) => {
    try {
        ...
        for (let i = 0; i < courses.length; i++) {
            ...
            const students = await studentModel.find({ courses: { $in: [courses[i]] } }).select('name')
            studentLists[i][courses[i]] = [...students]
        }
        res.send({ message: "success", data: studentLists })
    } catch (error) {
        console.log(error)
        return res.status(500).json({ message: "Error in get course students controller", data: error })
    }
}
...
const getSpecificCourseStudents = async (req, res) => {
    try {
        ...
        const students = await studentModel.find({ courses: { $in: [courseName] } }).select('name')
...
const selectFacultyCourses = async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({ message: "Error in select faculty courses controller", data: error })
    }
}

module.exports = { selectFacultyCourses, getCourseStudents, getSpecificCourseStudents }